...
try:
    ...
    with c2:
        # Menangani pembagian nol jika data kosong
        if not filtered_df.empty:
            avg_rating_df = filtered_df.groupby(chart_col)['Rating'].mean().reset_index()
            fig_bar = px.bar(avg_rating_df, x=chart_col, y='Rating', color='Rating', title="Kualitas Layanan Kompetitor")
            st.plotly_chart(fig_bar)
        else:
            st.write("Data tidak ditemukan untuk visualisasi grafik.")
    ...
    if model_ai:
        ...
        if user_msg:
            ...
            with st.chat_message("assistant"):
                try:
                    prompt = f"Data UMKM: {filtered_df[chart_col].value_counts().to_dict()}. Pertanyaan: {user_msg}"
                    response = model_ai.generate_content(prompt)
                    st.write(response.text)
                except Exception as e:
                    st.error("AI sedang sibuk, silakan coba lagi.")

# PENUTUP BLOK TRY UTAMA (Ini yang tadi hilang/error)
